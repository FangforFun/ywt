...
import com.gkzxhn.gkprison.userport.view.FullLinearLayout;
...
public class HomeFragment extends BaseFragmentNew{
    ...
    private List<News> focus_news_list;// 焦点新闻集合
    private List<News> allNews;// 所有的新闻集合
    ...
    private void setCacheNews() {
        ...
        allNews = new ArrayList<>();
        for (String key : keys){
            News news = FileUtils.getCacheNews(getActivity(), key);
            if (news != null){
                allNews.add(news);
                if(news.getIsFocus()){
                    focus_news_list.add(news);
