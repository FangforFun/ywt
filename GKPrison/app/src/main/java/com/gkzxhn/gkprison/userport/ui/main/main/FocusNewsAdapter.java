...
public class FocusNewsAdapter extends RecyclerView.Adapter<FocusNewsAdapter.MyViewHolder> {
    ...
    private List<News> focusNewsList;
    private Context mContext;
    private boolean isCache;

    public FocusNewsAdapter(List<News> list, Context context){
    ...
    public void onBindViewHolder(MyViewHolder holder, final int position) {
        Picasso.with(mContext).load(Constants.RESOURSE_HEAD + focusNewsList
                .get(position).getImage_url()).error(R.drawable.default_img).into(holder.iv_news_icon);
